...
**典型工具能力（示例，按你的目录结构自行命名）：**

| 工具类型 | 用途 |
|----------|------|
| 按日期查日程 | 从本地 markdown 解析某天的会议/待办（非系统日历） |
| 按章节读笔记 | 按 `##` / `###` 标题精确读取，避免整文件截断 |
| 列出目录索引 | 告诉 LLM 有哪些文件、章节可查 |
| 关键词搜索 | 在允许目录内 grep，返回原文片段 |

**验收**：问「明天有什么安排」「某笔记第三章写了什么」，回答与本地文件一致，非模型编造。

### 阶段 3 · 声纹认人（可选）

1. 单独部署 [voiceprint-api](https://github.com/xinnan-tech/voiceprint-api)（约 8005），连 MySQL。
2. 智控台 → **系统功能配置** → 勾选「声纹识别」。
3. `server.voice_print` 填容器内地址（见踩坑 #5）。
4. 智能体开启 **上报文字+语音** → 先语音聊几句 → **声纹识别 → 新增** 注册说话人。
5. 重启对话服务 / 板子后，日志应出现声纹已启用（非「声纹识别功能未启用」）。

**验收**：注册者问「你知道我是谁吗」能认出；未注册者不被误称为已注册身份（需人设配合）。
...
| # | 现象 | 原因 | 处理 |
|---|------|------|------|
| 5 | 声纹地址保存转圈 | 智控台校验 URL 时访问 `172.x` 热点 IP 超时 | 改用 `http://voiceprint-api:8005/voiceprint/health?key=<你的key>`（Docker 容器名） |
...
| 9 | 未注册者也被叫固定昵称 | 人设写死称呼，声纹未生效 | 声纹注册 + 人设约束「未知说话人用泛称」 |
...
| 11 | 工具结果上「加戏」编造 | LLM 幻觉 | 工具返回加「禁止编造」前缀；人设约束；调低 temperature |
| 12 | 把截止日期说成会议 | 模型混淆不同类型条目 | 各工具只返回单一类型；在返回里标注字段含义 |
| 13 | 章节内容答错 | 旧工具只读文件前 N 字符 | 按 markdown 标题精确读取目标章节 |
| 14 | 查不到未来日期 | 工具只读「今日」段落 | 增加按日期解析的专用工具 |
...
- MCP 白名单内的本地笔记原文、个人路径、MAC 地址等设备标识
...
*建档：2026-06-25 · 基于 Mac + ESP32-S3 WiFi 板 + xiaozhi-esp32-server 0.9.x 私有化实践整理*
